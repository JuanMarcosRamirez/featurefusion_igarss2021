function Cest = admm(Yh, Ym, lmbTV, Ne, varargin)
% ========================================================
...
%         X2    = soft(HhX1 + G2, lmbTV/rho);
%         X3    = soft(HvX1 + G3, lmbTV/rho);        
        ...
        if mod(t1, 2) == 1
            ...
            r_norm   = norm([Tmp1; Tmp2; Tmp3] ,'fro')^2/...
                       norm([X1;X2;X3], 'fro')^2;            
            s_norm   = norm([X1 - X1old;...
                             X2 - X2old;...
                             X3 - X3old], 'fro')^2/...
                       norm([G1;G2;G3], 'fro')^2;
...
function err = objective(Yh, Ym, Phih, HhC, HvC, Qh, Qm, C, lmb, lmbTV)
    ...
    err   = norm(Yh - Phih(C)*Qh, 'fro')^2 +...
            lmb*norm(Ym - C*Qm, 'fro')^2 + lmbTV*errTm;
end
